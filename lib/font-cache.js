...
if (process.platform === 'win32') {
    const fontPaths = fs.readdirSync(path.join(process.env.windir, 'fonts')).map(e => path.join(process.env.windir, 'fonts', e));
    ...
    try {
        Object.assign(
            fontCache,
            JSON.parse(
                zlib.brotliDecompressSync(fs.readFileSync(fontCachePath))
            ),
        );
    } catch (error) {
        /**
